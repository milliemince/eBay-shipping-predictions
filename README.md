...
We have also begun feature engineering, and have used the `item_zip` and `buyer_zip` to engineer a new feature: `zip_distance`, which quantifies the distance between the two zipcodes. After meeting with our TA, we've decided that a better stategy may be to look in to libraries that quantify the route time between two zip codes, rather than just the linear distance between them. 
